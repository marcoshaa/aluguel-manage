...
$mesAtual = date('Y-m');
$ativosStmt = $db->query("SELECT id, dia_vencimento FROM contratos WHERE ativo=1 AND imovel_id IN ({$inClause})");
...
if ($action === 'pagar') {
    ...
    $stPerm = $db->prepare("SELECT c.imovel_id FROM pagamentos p JOIN contratos c ON c.id=p.contrato_id WHERE p.id=?");
    $stPerm->execute([$pagId]);
    $rowPerm = $stPerm->fetch();
    if ($rowPerm) {
        $perm = permissoesImovel($userId, (int)$rowPerm['imovel_id']);
        if (!$perm['pode_escrever']) { flash('Sem permissão para registrar pagamento.', 'error'); redirect('pagamentos.php'); }
    }
...
$pagamentos = $db->prepare("
    SELECT p.*, c.dia_vencimento, c.valor_mensal, c.imovel_id,
           q.nome AS inquilino, i.endereco
    FROM pagamentos p
    JOIN contratos c ON c.id = p.contrato_id
    JOIN inquilinos q ON q.id = c.inquilino_id
    JOIN imoveis i ON i.id = c.imovel_id
    WHERE p.mes_referencia = ? AND c.imovel_id IN ({$inClause})
...
<div class="table-wrap">
    ...
    <table>
        ...
        <?php foreach ($pagamentos as $pg):
            $perm = permissoesImovel($userId, (int)$pg['imovel_id']);
        ?>
        <tr>
            <td><?= h($pg['inquilino']) ?></td>
            <td><?= h($pg['endereco']) ?></td>
            <td>Dia <?= $pg['dia_vencimento'] ?></td>
            <td><?= $perm['ver_valor'] ? moeda((float)$pg['valor_mensal']) : '—' ?></td>
            <td><?= ($perm['ver_pagamento'] && $pg['valor_pago']) ? moeda((float)$pg['valor_pago']) : ($pg['valor_pago'] ? '—' : '—') ?></td>
            <td><?= $pg['data_pagamento'] ? dataBR($pg['data_pagamento']) : '—' ?></td>
            <td><?= $perm['ver_pagamento'] ? statusPagBadge($pg['status']) : '—' ?></td>
            <td>
                <?php if ($perm['pode_escrever'] && $pg['status'] !== 'pago'): ?>
                ...
                <?php elseif ($perm['pode_escrever'] && $pg['status'] === 'pago'): ?>
    ...
    <?php else: ?>
    <div class="empty"><p>Nenhuma cobrança para este mês.</p></div>
    <?php endif ?>
</div>
